...
class Parser:
    ...
    def load_site(self, day):
        response = requests.get(
            "https://xn----3-iddzneycrmpn.xn--p1ai/lesson_table_show/", params={'day': day})
    ...
    def check_lessons(self, bot):
        ...
        sended_groups = [sch.group for sch in SendedGroups.objects.filter(
            date=self.get_offset_date())]
        day = self.get_day()
        ...
        if not self.today_sended:
            for group in schedule:
                if group['title'] not in sended_groups:
                    ...
                    SendedGroups.objects.get_or_create(
                        group=group["title"], date=self.get_offset_date())
            ...
            for idx, group in enumerate(schedule):
                if cache[self.today-1][idx] != group:
                    bot.mailing_schedule(
                        group, f'Изменения в рассписании "{day}" ')
                    SendedGroups.objects.get_or_create(
                        group=group["title"], date=self.get_offset_date())
