...
class Parsing_bot:
    ...
    def get_groups(self, profile, *args, **kargs):
        self.send_msg(profile.external_id, "Доступные группы:\n" + "\n".join(
            [f"{idx+1}. {val}" for idx, val in enumerate(self.pars.get_groups())]))
    ...
    def subscribe(self, profile, subs, *args, **kargs):
        ...
        if response:
            ...
            self.send_msg(profile.external_id,
                          VkBotMessages.NO_GROUP_FOR_SUB.value)
    ...
    def unsubscribe(self, profile, unsubs, *args, **kargs):
        ...
        if response:
            ...
            self.send_msg(profile.external_id,
                          VkBotMessages.NO_GROUP_FOR_UNSUB.value)
